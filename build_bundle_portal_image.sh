...
function check_usage {
    if [ ! -n "${LIFERAY_DOCKER_RELEASE_FILE_URL}" ]; then
        echo "Usage: ${0} --push"
        echo ""
...
function check_release {
	if [[ ${RELEASE_FILE_NAME} == *-dxp-* ]] || [[ ${RELEASE_FILE_NAME} == *-private* ]]; then
		DOCKER_IMAGE_NAME="dxp"
		DOCKER_LABEL_NAME="Liferay DXP"
	elif [[ ${RELEASE_FILE_NAME} == *-portal-* ]]; then
		DOCKER_IMAGE_NAME="liferay-portal"
...
function update_patching_tool {
	if [ -e "${TEMP_DIR}/liferay/patching-tool" ]; then
		...
		if [ ! -n "${patching_tool_minor_version}" ]; then
		...
		if (! echo ${patching_tool_minor_version} | grep -e '[0-9]*[.][0-9]*' >/dev/null); then
...
function build_docker_image {
	...
	if [[ ${LIFERAY_DOCKER_RELEASE_FILE_URL} == https://releases-* ]] || [[ ${LIFERAY_DOCKER_RELEASE_FILE_URL} == *releases.liferay.com* ]]
	then
		DOCKER_IMAGE_NAME=${DOCKER_IMAGE_NAME}-snapshot
	fi
	...
	then
		DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}hieunn08/${DOCKER_IMAGE_NAME}:latest")
	fi
